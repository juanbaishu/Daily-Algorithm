...
using namespace std;
ll a, b, c, res;
ll f[28][28][28];	// 记忆化搜索记录

ll work(ll a, ll b, ll c) {
	if (a <= 0 || b <= 0 || c <= 0) return 1;
	else if (a > 20 || b > 20 || c > 20) {
		if (f[20][20][20] == 0) f[20][20][20] = work(20, 20, 20);
		return f[20][20][20];
	}
	else if (a < b && b < c) {
		if (f[a][b][c - 1] == 0) f[a][b][c - 1] = work(a, b, c - 1);
		if (f[a][b - 1][c - 1] == 0) f[a][b - 1][c - 1] = work(a, b - 1, c - 1);
		if (f[a][b - 1][c] == 0) f[a][b - 1][c] = work(a, b - 1, c);
		return f[a][b][c - 1] + f[a][b - 1][c - 1] - f[a][b - 1][c];
	}
	else {
		if (f[a - 1][b][c] == 0) f[a - 1][b][c] = work(a - 1, b, c);
		if (f[a - 1][b - 1][c] == 0) f[a - 1][b - 1][c] = work(a - 1, b - 1, c);
		if (f[a - 1][b][c - 1] == 0) f[a - 1][b][c - 1] = work(a - 1, b, c - 1);
		if (f[a - 1][b - 1][c - 1] == 0) f[a - 1][b - 1][c - 1] = work(a - 1, b - 1, c - 1);
		return f[a - 1][b][c] + f[a - 1][b - 1][c] + f[a - 1][b][c - 1] - f[a - 1][b - 1][c - 1];
	}
}
